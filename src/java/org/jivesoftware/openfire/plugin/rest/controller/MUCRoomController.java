...
public class MUCRoomController {
	...
	/**
	 * Names of MUC rooms _should_ be node-prepped. This, however, was not
	 * guaranteed the case in some versions of Openfire and this plugin. Earlier
	 * versions of this plugin used a case-insensitive lookup to work around this.
	 * As this _should_ be unneeded, and is quite resource intensive, this behavior
	 * has been made configurable (disabled by default).
	 */
	public static final SystemProperty<Boolean> ROOM_NAME_CASE_INSENSITIVE_LOOKUP_ENABLED = SystemProperty.Builder
			.ofType(Boolean.class).setPlugin("REST API").setKey("plugin.restapi.muc.case-insensitive-lookup.enabled")
			.setDefaultValue(false).setDynamic(true).build();
	 ...
	 *
	 * This method returns any room that matches the provided room name
	 * case-insensitively, but prefers a case-sensitive match. It will <em>not</em>
	 * evaluate more than one service, in case more than one match the provided
	 * service name case-insensitively.
	 *
	 * @param serviceName The name of the service that contains the chat room.
	 * @param roomName    The name of the chat room.
	 * @return The chat room instance
	 * @throws ServiceException When no service for the provided name exists, or
	 *                          when that service does not contain a chat room of
	 *                          the provided name.
	 */
	@Nonnull
	protected static MUCRoom getRoom(@Nonnull final String serviceName, @Nonnull final String roomName)
		...
		// Names of MUC rooms _should_ be node-prepped. This, however, was not
		// guaranteed the case in some versions of Openfire and this plugin.
		// Earlier versions of this plugin used a case-insensitive lookup to work around
		// this. As this _should_ be unneeded, and is quite
		// resource intensive, this behavior has been made configurable (disabled by
		// default).
		if (room == null && ROOM_NAME_CASE_INSENSITIVE_LOOKUP_ENABLED.getValue()) {
			// As a last resort (because it's resource intensive), iterate over all
			// room(name)s for this service.
			for (String name : service.getAllRoomNames()) {
				if (name.equalsIgnoreCase(roomName) && (room = service.getChatRoom(roomName)) != null) {
					LOG.info(
							"Could not find a case-sensitive match for room '{}', but did find a case-insensitive match: '{}'. Case insensitive lookups are resource intensive. Verify that your database contains properly node-prepped MUC room names.",
							roomName, room.getName());
					return room;
				} else if (JID.nodeprep(name).equalsIgnoreCase(roomName)
						&& (room = service.getChatRoom(roomName)) != null) {
					LOG.info(
							"Could not find a case-sensitive match for room '{}', but did find a case-insensitive match after node-prepping: '{}'. Case insensitive lookups are resource intensive. Verify that your database contains properly node-prepped MUC room names.",
							roomName, room.getName());
					return room;
				}
			}
		...
		if (room == null) {
			throw new ServiceException("Chat room does not exist or is not accessible.", roomName,
					ExceptionType.ROOM_NOT_FOUND, Response.Status.NOT_FOUND);
		}
		return room;
	}
	...
	public MUCRoomEntities getChatRooms(String serviceName, String channelType, String roomSearch, boolean expand)
		...
		for (MUCRoomSearchInfo roomInfo : roomsInfo) {
			...
			if (chatRoom == null) {
				LOG.warn(
						"Cannot get room '{}' from service '{}' even though service's 'getAllRoomNames()' returns this name.",
	 ...
	 * @throws ServiceException the service exception
	 */
	public void createChatRoom(String serviceName, MUCRoomEntity mucRoomEntity, boolean sendInvitations)
			throws ServiceException {
	 ...
	 *
	 * @param serviceName     the service name
	 * @param mucRoomEntities the chat rooms to create
	 * @param sendInvitations whether to send invitations to affiliated users
	 * @return a report detailing which creates were successful and which weren't
	 * @throws ServiceException the service exception
	 */
	public RoomCreationResultEntities createMultipleChatRooms(String serviceName, MUCRoomEntities mucRoomEntities,
			boolean sendInvitations) throws ServiceException {
		...
		for (MUCRoomEntity roomToCreate : roomsToCreate) {
			...
			try {
				...
				result.setMessage(
						"Room creation failed due to " + e.getClass().getSimpleName() + ": " + e.getMessage());
			}
			results.add(result);
		}
	...
	public void updateChatRoom(String roomName, String serviceName, MUCRoomEntity mucRoomEntity,
		...
		try {
			...
			throw new ServiceException("Could not update the channel", roomName, ExceptionType.NOT_ALLOWED,
	 ...
	 */
	private void createRoom(MUCRoomEntity mucRoomEntity, String serviceName, boolean sendInvitations)
			throws NotAllowedException, ForbiddenException, ConflictException, AlreadyExistsException,
			ServiceException {
		...
		if (mucRoomEntity.getOwners() != null && mucRoomEntity.getOwners().size() > 0) {
			...
			log("Room '" + mucRoomEntity.getRoomName()
					+ "' is being created/updated without an owner. Adding default owner (as having an owner is non-optional).");
		...
		final Set<String> duplicates = allRequestedAffiliations.stream().filter(n -> !uniques.add(n)).map(JID::toString)
				.collect(Collectors.toSet());
		if (!duplicates.isEmpty()) {
			throw new ConflictException("The requested room defines duplicate affiliations for the following entities: "
		...
		if (!serviceRegistered) {
			...
			XMPPServer.getInstance().getMultiUserChatManager().createMultiUserChatService(serviceName, serviceName,
					false);
		}
		...
		if (mucRoomEntity.getBroadcastPresenceRoles() != null) {
			...
			room.setRolesToBroadcastPresence(new ArrayList<MUCRole.Role>());
		}

		// Set all roles
		log("Setting roles for room that is being " + (room.isLocked() ? "created" : "updated") + ": "
				+ mucRoomEntity.getRoomName());
		...
		log("Unlocking room that is being " + (room.isLocked() ? "created" : "updated") + ": "
				+ mucRoomEntity.getRoomName());
	...
	private boolean equalToAffiliations(MUCRoom room, MUCRoomEntity mucRoomEntity) {
		...
		Set<String> roomOwners = new HashSet<>(MUCRoomUtils.convertJIDsToStringList(owners)); // convertJIDsToStringList
																								// ignores group JIDs.
		...
		Set<String> entityOwners = mucRoomEntity.getOwners() != null ? new HashSet<>(mucRoomEntity.getOwners())
				: new HashSet<>();
		Set<String> entityAdmins = mucRoomEntity.getAdmins() != null ? new HashSet<>(mucRoomEntity.getAdmins())
				: new HashSet<>();
		Set<String> entityMembers = mucRoomEntity.getMembers() != null ? new HashSet<>(mucRoomEntity.getMembers())
				: new HashSet<>();
		Set<String> entityOutcasts = mucRoomEntity.getOutcasts() != null ? new HashSet<>(mucRoomEntity.getOutcasts())
				: new HashSet<>();
		...
		Set<String> entityOwnerGroups = mucRoomEntity.getOwnerGroups() != null
				? new HashSet<>(mucRoomEntity.getOwnerGroups())
				: new HashSet<>();
		Set<String> entityAdminGroups = mucRoomEntity.getAdminGroups() != null
				? new HashSet<>(mucRoomEntity.getAdminGroups())
				: new HashSet<>();
		Set<String> entityMemberGroups = mucRoomEntity.getMemberGroups() != null
				? new HashSet<>(mucRoomEntity.getMemberGroups())
				: new HashSet<>();
		Set<String> entityOutcastGroups = mucRoomEntity.getOutcastGroups() != null
				? new HashSet<>(mucRoomEntity.getOutcastGroups())
				: new HashSet<>();

		return entityOwners.equals(roomOwners) && entityAdmins.equals(roomAdmins) && entityMembers.equals(roomMembers)
				&& entityOutcasts.equals(roomOutcasts) && entityOwnerGroups.equals(roomOwnerGroups)
				&& entityAdminGroups.equals(roomAdminGroups) && entityMemberGroups.equals(roomMemberGroups)
	...
	/**
	 * Invites the user(s) or group(s) to the MUC room. This method differs from the
	 * other 'sendInvitations' methods in that no checks are performed. This really
	 * just sends the invitation stanza(s).
	 *
	 * @param serviceName          the service name
	 * @param roomName             the room name
	 * @param mucInvitationsEntity the invitation entity containing invitation
	 *                             reason and jids to invite
	 * @throws ServiceException the service exception
	 */
	public void inviteUsersAndOrGroups(String serviceName, String roomName, MUCInvitationsEntity mucInvitationsEntity)
		...
		for (JID jid : targetJIDs) {
			try {
				...
				throw new ServiceException("Could not invite user", jid.toString(), ExceptionType.NOT_ALLOWED,
						Response.Status.FORBIDDEN, e);
			}
		}
	}
	...
	/**
	 * Sends invitations "from the room" to a single user that is affiliated to the
	 * room.
	 *
	 * @see #sendInvitationsFromRoom(MUCRoom, EnumSet, Collection, String, boolean)
	 *
	 * @param room                    The room
	 * @param affiliations            The set of affiliations for which to send
	 *                                invitations, with a default of
	 *                                admin+owner+member when left unspecified
	 *                                (null)
	 * @param limitToThisUserOrGroup  The user or group for which to send
	 *                                invitations
	 * @param reason                  The reason to include in the invitation, with
	 *                                a sensible default when left unspecified
	 *                                (null)
	 * @param performAffiliationCheck Whether to validate if the user or group is
	 *                                actually affiliated to the room in the correct
	 *                                way
	 * @throws ForbiddenException The forbidden exception
	 */
	private void sendInvitationsToSingleJID(MUCRoom room, EnumSet<MUCRole.Affiliation> affiliations,
			JID limitToThisUserOrGroup, String reason, boolean performAffiliationCheck) throws ForbiddenException {
	...
	/**
	 * Sends invitations "from the room" to users that are affiliated to the room.
	 * The target audience can be limited to a specific set of JIDs through the
	 * #limitToTheseUsers parameter. If this parameter is left null, invitations are
	 * sent to all users with the specified affiliations.
	 *
	 * Before sending any invitation, this method checks whether the invitation
	 * recipient is actually affiliated to the room in the way that the invitation
	 * expresses.
	 *
	 * @param room                    The room
	 * @param affiliations            The set of affiliations for which to send
	 ...
	 *                                admin+owner+member when left unspecified
	 *                                (null)
	 * @param limitToTheseUsers       The collection of users for which to send
	 *                                invitations, with a default of "all"
	 *                                affiliated users when left unspecified (null)
	 * @param reason                  The reason to include in the invitation, with
	 *                                a sensible default when left unspecified
	 *                                (null)
	 * @param performAffiliationCheck Whether to validate if the user or group is
	 ...
	 * @throws ForbiddenException The forbidden exception
	 */
	private void sendInvitationsFromRoom(MUCRoom room, EnumSet<MUCRole.Affiliation> affiliations,
			Collection<JID> limitToTheseUsers, String reason, boolean performAffiliationCheck)
		...
		if (affiliations.contains(MUCRole.Affiliation.admin)) {
			...
			for (JID roomAdmin : sendHere) {
				sendSingleInvitationFromRoom(roomAdmin, room, roomRole, MUCRole.Affiliation.admin,
		...
		if (affiliations.contains(MUCRole.Affiliation.owner)) {
			...
			for (JID roomOwner : sendHere) {
				sendSingleInvitationFromRoom(roomOwner, room, roomRole, MUCRole.Affiliation.owner,
		...
		if (affiliations.contains(MUCRole.Affiliation.member)) {
			...
			for (JID roomMember : sendHere) {
				sendSingleInvitationFromRoom(roomMember, room, roomRole, MUCRole.Affiliation.member,
	...
	/**
	 * Sends an invitation for a specific affiliation to a single JID, (optionally)
	 * performing a check if that JID is actually affiliated to the room that way.
	 *
	 * @param sendHere         The JID to send the invitation to
	 * @param room             The room
	 * @param roomRole         Role of the room (added for optimisation, to prevent
	 *                         the MUCRole.createRoomRole(room) from being called
	 *                         many times)
	 * @param affiliation      The affiliation for which the jid is invited
	 * @param invitationReason The reason to include in the invitation message
	 * @param validation       Function to apply to the room and the jid to check
	 *                         whether the jid is actually affiliated in the correct
	 *                         way (or null if no validation is required)
	 * @throws ForbiddenException The forbidden exception
	 */
	private void sendSingleInvitationFromRoom(JID sendHere, MUCRoom room, MUCRole roomRole,
			MUCRole.Affiliation affiliation, String invitationReason, BiFunction<MUCRoom, JID, Boolean> validation)
			throws ForbiddenException {
		boolean jidIsGroup = false;

		if (validation != null && !validation.apply(room, sendHere)) {
			log("User or group " + sendHere + " can not be invited to be " + affiliation + " of room " + room.getName()
					+ " because it is not affiliated that way");
		} else {
			// First handle group behavior
			Group g = UserUtils.getGroupIfIsGroup(sendHere);
			if (g != null) {
				jidIsGroup = true;
				// This is a group jid, so we need to send the invitation to every single group
				// member
				for (JID singleGroupMemberJID : g.getAll()) {
					// Skip affiliation check, because it has already been done for the group, and
					// the single user may not
					// actually be known to be affiliated on its own merits
					sendSingleInvitationFromRoom(singleGroupMemberJID, room, roomRole, affiliation, invitationReason,
							null);
				}
			}
	 ...
	 * @param room          the room
	 * @param mucRoomEntity the muc room entity
	 * @return all users for which a role was added
	 * @throws ForbiddenException  the forbidden exception
	 * @throws NotAllowedException the not allowed exception
	 * @throws ConflictException   the conflict exception
	 */
	private static Collection<JID> setRoles(MUCRoom room, MUCRoomEntity mucRoomEntity)
			throws ForbiddenException, NotAllowedException, ConflictException {
		final Collection<JID> allNewAffiliations = new ArrayList<>();

		// Calculate which affiliations are to be removed from the room. These are all
		// the old affiliations, that are now no longer affiliations.
		...
		affiliationsToReset.removeAll(newOwners); // Do not remove these from the room after we're done re-affiliating
													// everyone!
		newOwners.removeAll(room.getOwners()); // Removing the ones that are already associated. We don't need to add
												// these again.
		...
		affiliationsToReset.removeAll(newAdmins); // Do not remove these from the room after we're done re-affiliating
													// everyone!
		newAdmins.removeAll(room.getAdmins()); // Removing the ones that are already associated. We don't need to add
												// these again.
		...
		affiliationsToReset.removeAll(newMembers); // Do not remove these from the room after we're done re-affiliating
													// everyone!
		newMembers.removeAll(room.getMembers()); // Removing the ones that are already associated. We don't need to add
													// these again.
		...
		affiliationsToReset.removeAll(newOutcasts); // Do not remove these from the room after we're done re-affiliating
													// everyone!
		newOutcasts.removeAll(room.getOutcasts()); // Removing the ones that are already associated. We don't need to
													// add these again.
	...
	/**
	 * Returns a collection of user addresses of every user that has a particular
	 * affiliation to a room.
	 *
	 * @param serviceName the service name of the room
	 * @param roomName    the name of the room for which to return affiliated users
	 * @param affiliation the affiliation for which to return all users
	 * @return All users that have the specified affiliation to the specified room.
	 * @throws ServiceException On any issue looking up the room or its affiliated
	 *                          users.
	 */
	public Collection<JID> getByAffiliation(@Nonnull final String serviceName, @Nonnull final String roomName,
			@Nonnull final MUCRole.Affiliation affiliation) throws ServiceException {
		...
		default:
			return room.getOccupants().stream().filter(o -> affiliation.equals(o.getAffiliation()))
					.map(MUCRole::getUserAddress).collect(Collectors.toSet());
		}
	}

	/**
	 * Updates a list of users that have a particular affiliation to a room with a
	 * new list.
	 *
	 * This will remove all users as having an affiliation of this type with the
	 * room for users that are not in the list of replacements.
	 *
	 * @param serviceName     the service name of the room
	 * @param roomName        the name of the room for which to replace affiliated
	 *                        users
	 * @param affiliation     the affiliation for which to replace all users
	 * @param jids            the new list of affiliated users
	 * @param sendInvitations whether to send invitations to newly affiliated users
	 * @throws ServiceException On any issue looking up the room or changing its
	 *                          affiliated users.
	 */
	public void replaceAffiliatedUsers(@Nonnull final String serviceName, @Nonnull final String roomName,
			@Nonnull final MUCRole.Affiliation affiliation, boolean sendInvitations, @Nonnull final String... jids)
			throws ServiceException {
		...
		for (String replacement : jids) {
			try {
				...
				throw new ServiceException("Unable to parse value as jid: " + replacement, roomName,
		...
		// The users to add are the replacements that aren't already in the old
		// collection.
		...
		final MUCRoom room = getRoom(serviceName, roomName);
		try {
			// First, add all new affiliations (some affiliations aren't allowed to be
			// empty, so removing things first could cause issues).
			switch (affiliation) {
			case admin:
			...
			throw new ServiceException("Forbidden to apply modification to list of " + affiliation, roomName,
					ExceptionType.NOT_ALLOWED, Response.Status.FORBIDDEN, e);
		} catch (ConflictException e) {
			throw new ServiceException("Could not apply modification to list of " + affiliation, roomName,
					ExceptionType.NOT_ALLOWED, Response.Status.CONFLICT, e);
		}
		...
		try {
			...
			throw new ServiceException(
					"Can not send invitation to newly affiliated " + affiliation + " users or groups", roomName,
					ExceptionType.NOT_ALLOWED, Response.Status.FORBIDDEN, e);
		}
	}

	/**
	 * Adds to a list of users that have a particular affiliation to a room with a
	 * new list, without affecting the pre-existing list.
	 *
	 * @param serviceName     the service name of the room
	 * @param roomName        the name of the room for which to add affiliated users
	 * @param affiliation     the affiliation for which to add users
	 * @param jids            the list of additional affiliated users
	 * @param sendInvitations whether to send invitations to newly affiliated users
	 * @throws ServiceException On any issue looking up the room or changing its
	 *                          affiliated users.
	 */
	public void addAffiliatedUsers(@Nonnull final String serviceName, @Nonnull final String roomName,
			@Nonnull final MUCRole.Affiliation affiliation, boolean sendInvitations, @Nonnull final String... jids)
			throws ServiceException {
	 ...
	 * @param roomName    the room name
	 * @param jid         the jid
	 * @param affiliation the type of affiliation to remove. Null to remove any
	 *                    affiliation.
	 * @throws ServiceException the service exception
	 */
	public void deleteAffiliation(String serviceName, String roomName, MUCRole.Affiliation affiliation, String jid)
			throws ServiceException {
		MUCRoom room = getRoom(serviceName, roomName);
		try {
			...
			throw new ServiceException("Could not delete affiliation", jid, ExceptionType.NOT_ALLOWED,
					Response.Status.FORBIDDEN, e);
		} catch (ConflictException e) {
			throw new ServiceException("Could not delete affiliation", jid, ExceptionType.NOT_ALLOWED,
					Response.Status.CONFLICT, e);
			...
			throw new ServiceException("Could not delete affiliation", jid, ExceptionType.ILLEGAL_ARGUMENT_EXCEPTION,
	...
	public void addAdmin(String serviceName, String roomName, String jid) throws ServiceException {
		LOG.debug("MUCRoomController: addAdmin(): START : serviceName: " + serviceName + " roomName: " + roomName + " jid: " + jid);
		...
		try {
			...
			throw new ServiceException("Could not add admin", jid, ExceptionType.NOT_ALLOWED, Response.Status.FORBIDDEN,
					e);
		} catch (ConflictException e) {
			throw new ServiceException("Could not add admin", jid, ExceptionType.NOT_ALLOWED, Response.Status.CONFLICT,
					e);
		}
		
		LOG.debug("MUCRoomController: addAdmin(): END");
	}
	...
	public void deleteAffiliation(String serviceName, String roomName, String jid) throws ServiceException {
		...
		LOG.debug("MUCRoomController: deleteAffiliation(): END");
	}
	...
	public void addMember(String serviceName, String roomName, String jid) throws ServiceException {
		LOG.debug("MUCRoomController: addMember(): START : serviceName: " + serviceName + " roomName: " + roomName + " jid: " + jid);
		...
		try {
			...
			throw new ServiceException("Could not add member", jid, ExceptionType.NOT_ALLOWED,
					Response.Status.FORBIDDEN, e);
		}
		
		LOG.debug("MUCRoomController: addMember(): END");
	}
	 ...
	 * @throws ServiceException the service exception
	 */
	public void addMembers(String serviceName, String roomName, String jids) throws ServiceException {
		LOG.debug("MUCRoomController: addMember(): START : serviceName: " + serviceName + " roomName: " + roomName + " jid: " + jids);

		MUCRoom room = XMPPServer.getInstance().getMultiUserChatManager().getMultiUserChatService(serviceName)
				.getChatRoom(roomName.toLowerCase());

		if (!jids.contains(","))
			addMember(serviceName, roomName, jids);
		else
			for (String jid : jids.split(",")) {
				try {
					room.addMember(UserUtils.checkAndGetJID(jid), null, room.getRole());
				} catch (ForbiddenException | ConflictException e) {
					throw new ServiceException("Could not add member", jid, ExceptionType.NOT_ALLOWED,
	...
	public void addOutcast(String serviceName, String roomName, String jid) throws ServiceException {
		LOG.debug("MUCRoomController: addOutcast(): START : serviceName: " + serviceName + " roomName: " + roomName + " jid: " + jid);
		...
		try {
			...
			throw new ServiceException("Could not add outcast", jid, ExceptionType.NOT_ALLOWED,
					Response.Status.FORBIDDEN, e);
		} catch (ConflictException e) {
			throw new ServiceException("Could not add outcast", jid, ExceptionType.NOT_ALLOWED,
					Response.Status.CONFLICT, e);
		...
		LOG.debug("MUCRoomController: addOutcast(): END");
	}
	...
	public void addOwner(String serviceName, String roomName, String jid) throws ServiceException {
		LOG.debug("MUCRoomController: addOwner(): START : serviceName: " + serviceName + " roomName: " + roomName + " jid: " + jid);
		...
		try {
			...
			throw new ServiceException("Could not add owner", jid, ExceptionType.NOT_ALLOWED, Response.Status.FORBIDDEN,
					e);
		}
		
		LOG.debug("MUCRoomController: addOwner(): END");
	}
